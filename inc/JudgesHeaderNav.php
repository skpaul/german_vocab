...
class JudgesHeaderNav
{
    ...
    public static function prepare(array $params): string
    {
        ...
        if (isset($_GET["session"]) && !empty($_GET["session"])) {
            ...
            $loginHtml = <<<HTML
                <a href="$baseUrl/app/approval-authority/judge-logout.php{$session}"><span class="m-icons mr-025">article</span>Log out</a>
            HTML;
        } else {
            $session = "";
            //show login if session not found
            $loginHtml = <<<HTML
                <a class="c-red-4" href="$baseUrl/app/login/login.php"><span class="m-icons mr-025 c-teal-2">article</span><span class="c-teal-2">Login</span></a>
            HTML;
        }
                ...
                <div class="main-top-nav-container">
                    <!-- <div class="container"> -->
                        <nav id="mainTopNav" class="main-top-nav">
                            <a href="$baseUrl/app/approval-authority/dashboard.php{$session}"><span class="m-icons mr-025 ">home</span><span>Dashboard</span></a>    
                            <a href="$baseUrl/app/approval-authority/uploads/uploads.php{$session}"><span class="m-icons mr-025 ">description</span><span>Uploads</span></a>    
                            {$loginHtml}
                        </nav> 
                    <!-- </div> -->
                </div>
