<?php
    ...
    try {
        $id = $validable->label("ID")->post("id")->required()->asInteger(false)->validate();
        ...
        $data["articleId"] = $validable->label("Article")->post("article")->asInteger(true)->maxLen(5)->default(0)->validate();
    ...
    $sql = "SELECT id FROM words WHERE english=:english AND german=:german AND id<>:id";
    $isExist = $db->fetchAssoc($sql, ["english"=>$data["english"], "german"=>$data["german"], "id"=>$id ]);
    
    if($isExist)
        die($json->fail()->message("This word already exists.")->create());

    $sql = $db->prepareUpdateSql("words",$data,"id=:id",true);
